...
- **Never rewrite an applied migration.** Corrections go forward as new revisions. A plugin guard (`.opencode/plugins/guards.js`) enforces this mechanically; do not try to route around it.
...
The full checklist is `.opencode/skills/safe-migration/SKILL.md` — invoke it (or have the
