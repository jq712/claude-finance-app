...
schema change is Class B (CLAUDE.md's Risk classes) — implementation, then independent
...
- **Never edit a committed migration file.** `guard-applied-migrations.sh` (`PreToolUse` hook)
  blocks this mechanically — if you hit it, the fix is a new forward migration, not routing
  around the guard. `uv run alembic revision -m '<what this corrects>'`.
...
- **Plaid natural keys are unique-constrained**: `transaction_id`, `account_id`, `item_id` (or
  ...
  `.claude/skills/plaid-sync-review/SKILL.md` for the ingestion side of this same invariant.
...
- Run `pre-merge-review` (`.claude/skills/pre-merge-review/SKILL.md`) regardless — migrations are
