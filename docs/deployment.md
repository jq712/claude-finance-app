...
- **`finops deploy <sha>`** — validates `<sha>` looks like a Git SHA (refuses anything else; `latest` is never accepted here even though the image carries that tag too), records a `ops.releases` row (`status="deploying"`), runs `docker compose pull && docker compose up -d` with `RELEASE_ID=<sha>`, then checks `finops health`. Healthy → the release is promoted to `current` (the prior `current` becomes `previous`). Unhealthy → the release is marked `failed` and **`finops deploy` automatically rolls back** to the previous known-good release (ADR-008) before exiting non-zero.
...
- **`finops restart`** — `docker compose restart app` only; does not touch Postgres or change `ops.releases`. Use this for "the app process needs to come back," not "roll out a new release."
