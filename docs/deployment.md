...
Everything up to "an owner-performed action once the production directory is provisioned": the Docker image, the production Compose file, systemd units, the CI/CD pipeline through image publish/migration-preflight/staging-smoke, and the `finops deploy`/`rollback`/`restart` commands that will operate on the real deployment once it exists. **No production deployment is provisioned yet** — the VPS the engineering workspace runs on is also the intended production host (ADR-007/ADR-010, revised 2026-09-13; see `docs/security-model.md`'s "Trust boundaries"), but `docs/runbooks/deploy.md` §1's production Unix user, directory, and credentials haven't been set up on it. `.github/workflows/ci.yml`'s `production-deploy` job is a deliberate no-op gate that records release readiness in the job summary — it does not deploy anywhere or fake a deploy target. The first real deploy is a manual, owner-performed run of `finops deploy <sha>` on the production side of that boundary, documented step by step in `docs/runbooks/deploy.md`.
...
- **Production-side provisioning on the shared VPS** — the engineering workspace's own host is the intended production host (ADR-007/ADR-010, revised 2026-09-13), but the production Unix user, `/opt/finance-app`, `/etc/finance-app/credentials`, and the minted credentials themselves don't exist yet. No production credential material exists in the engineering session regardless — that boundary is now Unix-permission-based rather than host-based, per `docs/security-model.md`'s "Trust boundaries", and still holds. `docs/runbooks/deploy.md` documents the exact procedure.
