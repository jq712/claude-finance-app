...
| `finance_migrator` | DDL; creates the other five roles; INHERIT privilege in all five (migration `0007`, for `DROP OWNED BY` during downgrade) | `alembic upgrade` only |
...
Migration `0007` (Milestone 7) grants `finance_migrator` INHERIT privilege in all five roles. On PG 16+, `CREATEROLE` automatically confers role membership to the creator, but without the INHERIT option. Postgres's `DROP OWNED BY` command requires INHERIT privilege (has_privs_of_role), not just membership. `0002`'s downgrade loop runs `DROP OWNED BY` for all five roles, and fails without INHERIT. The grant is explicit (`WITH INHERIT TRUE, SET FALSE`) and one-way: `0007.downgrade()` is deliberately a no-op, not a symmetric revoke, because Alembic runs downgrades newest-first (so a revoke would execute before 0002's downgrade needs INHERIT), and because 0002's downgrade ends with `DROP ROLE IF EXISTS`, which destroys all membership and privilege grants automatically. See the migration's own docstring for the detailed reasoning.
