...
The exact release-copy mechanism (a plain `rsync`/`git archive` from a bare clone/`git worktree add`, and exactly how backup-then-migrate-then-symlink-then-restart gets scripted and where) is a follow-up implementation decision, not fixed by this ADR beyond the ordering above.
...
- **One less moving part**: no image registry, no image pull step, no container runtime to keep patched on the production host. A new failure mode in exchange: the release-copy step (`rsync`/`git archive`/whatever the follow-up session picks) must be as carefully gotten right as `docker pull` used to be — a partial or corrupted copy is now this project's own problem to guard against rather than the container runtime's.
