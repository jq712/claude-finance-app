...
| [008](ADR-008-immutable-releases-rollback.md) | Immutable container releases and rollback (mechanism superseded by ADR-019 — no containers; the "one rollback step" principle carries over to a symlink-based release) |
...
| [016](ADR-016-production-deployment-control-plane.md) | Production deployment control plane: Compose interpolation, no long-running `app` until Milestone 8, and release-promotion concurrency — **superseded by ADR-019** (no Docker; kept as historical record) |
...
| [019](ADR-019-bare-metal-no-docker-deployment.md) | Bare-metal deployment — no Docker; dev tree and production (`/opt/finance`) split on one shared VPS; host PostgreSQL with `finance_dev`/`finance_prod` |
