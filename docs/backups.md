...
All of this is `src/finance_app/ops/backup.py::create_backup()` — not shell one-liners — so it's unit-testable and there is exactly one place this logic lives. `deploy/scripts/backup.sh` is a two-line wrapper that runs it directly against `/opt/finance/current`'s virtualenv (ADR-019, revised 2026-09-13 — no Docker; supersedes the earlier `docker compose --profile backup run` mechanism), invoked by a systemd unit scoped to only the `finance_backup`/`BACKUP_ENCRYPTION_KEY` credentials it needs — so a backup always uses the exact code of the currently-deployed release (whatever `/opt/finance/current` points at).
...
2. Stand up a **throwaway** target — never the real `finance_prod` database, never even `finance_dev`. Under ADR-019's bare-metal design (revised 2026-09-13, no Docker) this needs a bare-metal equivalent of the old throwaway-container mechanism: plausibly a scratch database on the same host PostgreSQL instance (created fresh, dropped after), or a second `initdb`'d data directory — exact mechanism is a follow-up implementation decision.
...
6. Always tear the throwaway target down, success or failure (`trap cleanup EXIT INT TERM`).

This is a full, real restore into a real (if disposable) PostgreSQL target — not a checksum comparison, not a "the file exists" check.
...
The plaintext `pg_dump`/`pg_restore` output exists only briefly, on local disk under `BACKUP_DIR`, in a private per-run staging directory — removed immediately after GPG encryption succeeds, including in the failure path (`finally: shutil.rmtree(staging_dir, ignore_errors=True)` in `create_backup`/`restore_backup`, QA-10). A leftover staging directory from a process that didn't get to run its own `finally` (SIGKILL, OOM) is swept on the next backup/restore invocation (`_sweep_stale_staging_dirs`). The encrypted `.gpg` archive is the only artifact that persists.

## Deliberately deferred

- **Off-machine / cloud backup storage.** This milestone proves the backup/restore/verify *pipeline* end to end against `finance_dev` with synthetic data (satisfying this milestone's "backup restore test succeeds" exit criterion) and documents exactly what an off-machine target needs to satisfy: read/write access to `BACKUP_DIR`'s contents (or a hook in `deploy/scripts/backup.sh` to push the just-created archive after `finance_app.ops.backup.create_backup()` returns), and its own credential delivered the same way as every other secret. No cloud storage credentials are readable by the engineering session (ADR-010, revised 2026-09-13) and none were fabricated to simulate one; provisioning a real off-machine destination (object storage, a second host, etc.) is an owner decision and an owner-performed runbook step once a target is chosen.
...
- **Credential-in-environment exposure, generally.** Resolved as a Docker-specific concern by ADR-019 (2026-09-13, no Docker — the old gap was that `docker inspect`/a container shell could read a running container's env vars for its whole lifetime). The baseline risk of *any* process holding a credential in its own environment (readable via `/proc/<pid>/environ` by the same user or root) is not Docker-specific and still applies to whatever mechanism the bare-metal systemd units use to pass `BACKUP_ENCRYPTION_KEY`/`FINANCE_BACKUP_DB_PASSWORD`/etc. to the backup process — worth designing, when implemented, to read credential files directly from `$CREDENTIALS_DIRECTORY` (systemd's `LoadCredentialEncrypted=` decrypts to files there) rather than requiring an env-var export step. See `docs/security-model.md` invariant 4 for the full writeup.
