...
6. Never touches production. `finops deploy`/`rollback` stay an owner-performed action
   ...
   it is, the autonomous session's Unix user has no read access to the production directory or
   credentials (ADR-007/ADR-010, revised 2026-09-13 — engineering and production now share a
   VPS; see `docs/security-model.md`'s "Trust boundaries" for what actually enforces this).
